...
void main(void) {
   highp vec2 landTextureCoordBlend = fract(vTextureCoord);
   highp vec2 skyTextureCoordBlend = fract(vTextureCoord); //TODO use screen space
   highp vec2 textureCoordBlend = mix(landTextureCoordBlend, skyTextureCoordBlend, vSky);
   highp vec2 sampleCoords = mix(vec2(vAtlasCoord.rg), vec2(vAtlasCoord.ba), textureCoordBlend);
